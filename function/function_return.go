...
func rectangle2(len,wid float64)(peri float64,area float64){   //提前写好返回值列表
	peri = (len +wid)*2
	area = len*wid
	return
}
...
//定义一个函数，带返回值
func getSum()int { //返回值的类型，这里没有定义返回值的名称
	sum := 0
	for i:=1;i<=10;i++{
		sum += i
	}
...
func getSum2()(sum int){//定义函数时，指明要返回的数据是哪一个，返回值的名称和类型
	for i:=1;i<=100;i++{
		sum += i
	}
