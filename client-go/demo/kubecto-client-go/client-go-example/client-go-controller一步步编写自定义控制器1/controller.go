...
type controller struct {
	...
	depLister      appslisters.DeploymentLister    //列出deployment
	depCacheSynced cache.InformerSynced            //InformerSynced是一个函数，可以用来确定一个信息员是否已经同步。这对于确定缓存是否已经同步很有用。
	queue          workqueue.RateLimitingInterface //使用限速队列
}
...
func newController(clientset kubernetes.Interface, depInformer appsinformers.DeploymentInformer) *controller {
	...
	depInformer.Informer().AddEventHandler( //添加资源事件回调方法
		cache.ResourceEventHandlerFuncs{
			AddFunc:    handleAdd, //创建资源时触发这个方法
